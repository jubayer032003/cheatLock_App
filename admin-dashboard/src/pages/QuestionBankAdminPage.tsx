...
export function QuestionBankAdminPage({ initialTab = "questions", mode = "list" }: { initialTab?: Tab; mode?: "list" | "new" | "edit" }) {
  ...
  async function handleSaveQuestion() {
    ...
    await runSave(async () => {
      ...
      if (mode === "edit" || mode === "new") navigate("/question-bank", { replace: true });
    });
  }
  ...
  async function loadQuestionForEdit(id: string) {
    setQuestionLoading(true);
    setErrorMessage("");
    try {
      const question = await fetchQuestionBankQuestion(id);
      editQuestion(question);
    } catch (error) {
      setErrorMessage(readErrorMessage(error, "Could not load question for editing."));
    } finally {
      setQuestionLoading(false);
    }
  }
      ...
      {loading ? (
        ...
        <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
          ...
          <QuestionListPanel
            ...
            onEdit={(question) => navigate(`/question-bank/questions/${question.id}/edit`)}
